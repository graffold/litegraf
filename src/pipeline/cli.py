...
def _build_backends(args: argparse.Namespace, config: dict[str, Any]) -> tuple:
    ...
    def arg_value(name: str) -> Any:
        return getattr(args, name, None)

    graph_store = BackendRegistry.resolve_graph_store(
        arg_value("graph_backend") or config.get("graph_backend", "neo4j"),
        uri=arg_value("graph_uri") or config.get("graph_uri", "bolt://localhost:7687"),
        auth=(
            arg_value("graph_user") or config.get("graph_user", "neo4j"),
            arg_value("graph_password") or config.get("graph_password", "password"),
        ),
        database=arg_value("graph_database") or config.get("graph_database", "neo4j"),
    )
    embedding_provider = LocalEmbeddingProvider(
        model_name=arg_value("embedding_model")
        or config.get("embedding_model", "all-mpnet-base-v2")
    )

    llm_provider_name = arg_value("llm_provider") or config.get(
        "llm_provider", "ollama"
    )
    ...
    if llm_provider_name == "cloudflare":
        ...
        llm_provider = CloudflareLLMProvider(
            model=arg_value("llm_model")
            or config.get("llm_model", "@cf/meta/llama-3.1-8b-instruct"),
        )
        ...
        llm_provider = BedrockLLMProvider(
            model=arg_value("llm_model")
            or config.get("llm_model", "eu.amazon.nova-lite-v1:0"),
        )
    else:
        from pipeline.backends.ollama_llm import OllamaLLMProvider

        llm_provider = OllamaLLMProvider(
            model=arg_value("llm_model") or config.get("llm_model", "llama3"),
            base_url=arg_value("llm_url")
            or config.get("llm_url", "http://localhost:11434"),
        )
...
def main() -> None:
    ...
    run_parser.add_argument(
        "--graph-backend", default="neo4j", choices=["neo4j", "memgraph"]
    )
    ...
    run_parser.add_argument(
        "--llm-provider", default="ollama", choices=["ollama", "bedrock", "cloudflare"]
    )
