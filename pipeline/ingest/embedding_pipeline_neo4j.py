...
class Neo4jEmbeddingPipeline:
    def __init__(
        ...
        **kwargs,
        ...
        if service == "openai":
            ...
            if not api_key:
                raise ValueError("OPENAI_API_KEY must be set in config.py")
            embed_kwargs["api_key"] = api_key
            llm_kwargs["api_key"] = api_key
        elif service == "hf-inference":
            embed_type = "hf-inference"
            api_key = Config.get_config("HUGGINGFACE_API_KEY")
            if not api_key:
                raise ValueError("HUGGINGFACE_API_KEY must be set in config.py")
            embed_kwargs["api_key"] = api_key
            llm_kwargs["api_key"] = api_key
    ...
    def _initialize_retrievers(self):
        """Initialize retrievers after index creation."""
        try:
            # Use neo4j-graphrag retrievers with index validation
            with self.db.driver.session(database=self.database) as session:
                index_exists = session.run(
                    "SHOW INDEXES WHERE name = 'node_embeddings'"
                ).single()
                ...
                try:
                    ...
                    with self.db.driver.session(database=self.database) as session:
                        for node, embedding, node_text in zip(
                            ...
                            # Update existing node - conditionally store embedding as property
                            ...
                            if result.single():
                                ...
                                # If node doesn't exist, create it (shouldn't happen in normal flow)
                                logger.warning(
            ...
            for chunk in doc.chunks:
                try:
                    ...
                    for node in chunk.nodes:
                        ...
                        # Create full_text content for search - include the actual abstract text
                        full_text_parts = [
                            f"Type: {node['type']}",
                            f"Name: {node.get('name', 'unknown')}",
                            f"ID: {node['id']}",
                        ]
                        # Add the full abstract text as the primary content for full-text search
                        if chunk.text:
                            full_text_parts.append(f"Abstract: {chunk.text}")
                        full_text = " | ".join(full_text_parts)

                        # Optimize text storage - avoid redundant abstract_text if contained in full_text
                        if (
                            self.optimize_text_storage
                            and chunk.text
                            and chunk.text in full_text
                        ):
                            # Don't store abstract_text separately if it's already in full_text
                            abstract_text_to_store = None
                        else:
                            abstract_text_to_store = chunk.text

                        with self.db.driver.session(database=self.database) as session:
                            # First, try to update existing node - conditionally store embedding as property
                            if self.store_embedding_properties:
                            ...
                            if result.single():
                                ...
                                # If node doesn't exist, create it - conditionally store embedding as property
                                if self.store_embedding_properties:
                                    if abstract_text_to_store is not None:
                                        session.run(
                                            f"""
                            ...
                            # Ensure chunk-node relationship exists using the correct FROM_CHUNK pattern
                            session.run(
                                f"""
                                MATCH (n:{node_label} {{id: $id}})
                                MERGE (c:Chunk {{chunk_id: $chunk_id}})
                    ...
                    for rel in chunk.relationships:
                        ...
                        try:
                            with self.db.driver.session(
                                ...
                                # Create pattern ID for relationship consolidation
                                pattern_id = (
            ...
            for chunk in doc.chunks:
                try:
                    ...
                    with self.db.driver.session(database=self.database) as session:
                        ...
                        logger.debug(f"Stored chunk embedding: {chunk.chunk_id}")
